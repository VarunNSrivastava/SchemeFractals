...
def scheme_eval(expr, env, _ = None):
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            raise SchemeError('argument is not valid scheme procedure: {0}'.format(procedure))
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is not nil or vals is not nil:
            raise SchemeError("number of arguments mismatched with number of formal parameters.")
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("wrong number of arguments passed: {0}".format(*python_args))
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        symbol = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, symbol)
        return target.first
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    #if expressions.first is "'" or expressions.first is "quote":
    #    return Pair("quote", do_quote_form(expressions.rest.first))
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions.rest.first, 1)
